...
import net.minecraft.crash.CrashReportExtender.ThrowableProxy;
...
class ThrowableProxyRenderer {
	...
	static void formatWrapper(final StringBuilder sb, final ThrowableProxy cause, final List<String> ignorePackages,
		...
		formatElements(sb, EMPTY, cause.getCommonElementCount(),
	 ...
	 *
	 * @param src
	 *            ThrowableProxy instance to format
	 * @param sb
	 *            Destination.
	...
	static void formatExtendedStackTraceTo(final ThrowableProxy src, final StringBuilder sb, final List<String> ignorePackages, final TextRenderer textRenderer, final String suffix, final String lineSeparator) {
		...
		formatElements(sb, EMPTY, 0, causedTrace, src.getExtendedStackTrace(), ignorePackages, textRenderer, suffix, lineSeparator);
		formatSuppressed(sb, TAB, src.getSuppressedProxies(), ignorePackages, textRenderer, suffix, lineSeparator);
		formatCause(sb, EMPTY, src.getCauseProxy(), ignorePackages, textRenderer, suffix, lineSeparator);
	}
	 ...
	 *
	 * @param src
	 *            Throwable whose cause to render
	 * @param sb
	 *            Destination to render the formatted Throwable that caused this Throwable onto.
	 * @param ignorePackages
	 *            The List of packages to be suppressed from the stack trace.
	 * @param textRenderer
	 *            The text renderer.
	 * @param suffix
	 *            Append this to the end of each stack frame.
	...
	static void formatCauseStackTrace(final ThrowableProxy src, final StringBuilder sb, final List<String> ignorePackages, final TextRenderer textRenderer, final String suffix, final String lineSeparator) {
		...
		ThrowableProxyRenderer.renderSuffix(suffix, sb, textRenderer);
		textRenderer.render(lineSeparator, sb, "Text");
		ThrowableProxyRenderer.formatElements(sb, EMPTY, 0, src.getStackTrace(), src.getExtendedStackTrace(),
