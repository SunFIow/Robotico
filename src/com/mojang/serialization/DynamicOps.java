...
public interface DynamicOps<T> {
	...
	default DataResult<T> mergeToMap(final T map, final MapLike<T> values) {
		...
		values.entries().forEach(entry -> result.setValue(result.getValue().flatMap(r -> mergeToMap(r, entry.getLeft(), entry.getRight()))));
	...
	default DataResult<Consumer<BiConsumer<T, T>>> getMapEntries(final T input) {
		return getMapValues(input).map(s -> c -> s.forEach(p -> c.accept(p.getLeft(), p.getRight())));
	}
	...
	default DataResult<MapLike<T>> getMap(final T input) {
		return getMapValues(input).flatMap(s -> {
			try {
				return DataResult.success(MapLike.forMap(s.collect(Collectors.toMap(Pair::getLeft, Pair::getRight)), this));
	...
	default <U> U convertMap(final DynamicOps<U> outOps, final T input) {
		return outOps.createMap(getMapValues(input).result().orElse(Stream.empty()).map(e -> Pair.of(convertTo(outOps, e.getLeft()), convertTo(outOps, e.getRight()))));
	}
}
