...
import net.minecraft.util.DataFixUtils;
...
public class Dynamic<T> extends DynamicLike<T> {
	...
	public DataResult<Map<Dynamic<T>, Dynamic<T>>> getMapValues() {
		return ops.getMapValues(value).map(map -> {
			...
			map.forEach(entry -> builder.put(new Dynamic<>(ops, entry.getLeft()), new Dynamic<>(ops, entry.getRight())));
	...
	public Dynamic<T> updateMapValues(final Function<Pair<Dynamic<?>, Dynamic<?>>, Pair<Dynamic<?>, Dynamic<?>>> updater) {
		return DataFixUtils.orElse(getMapValues().map(map -> map.entrySet().stream().map(e -> {
			...
			return Pair.of(pair.getLeft().castTyped(ops), pair.getRight().castTyped(ops));
		}).collect(Collectors.toMap(Pair::getLeft, Pair::getRight))).map(this::createMap).result(), this);
	}
	...
	public DataResult<Stream<Pair<Dynamic<T>, Dynamic<T>>>> asMapOpt() {
		return ops.getMapValues(value).map(s -> s.map(p -> Pair.of(new Dynamic<>(ops, p.getLeft()), new Dynamic<>(ops, p.getRight()))));
	}
	...
	public <A> DataResult<Pair<A, T>> decode(final Decoder<? extends A> decoder) {
		return decoder.decode(ops, value).map(p -> Pair.of(p.getLeft(), p.getRight()));
	}
