...
//				lifecycle);
		Function<? super PartialResult<R>, ? extends PartialResult<T>> f2 = r -> new PartialResult<>(r.message, r.partialResult.map(function));
		Either<T, PartialResult<T>> either = result.mapBoth(function, f2);
		return create(either, lifecycle);
	}
...
//		});
		Function<R, DataResult<R>> left = r -> new DataResult<>(Either.left(r), lifecycle);
		Function<PartialResult<R>, DataResult<R>> right = r -> {
			onError.accept(r.message);
			Function<R, DataResult<R>> pR = pr -> new DataResult<>(Either.left(pr), lifecycle);
			Supplier<DataResult<R>> orElse = () -> create(Either.right(r), lifecycle);
			return r.partialResult
					.map(pR)
					.orElseGet(orElse);
				...
				r -> r.partialResult
						.map(value -> {
							...
							return create(Either.right(second.get().map(
									...
									r2 -> new PartialResult<>(appendMessages(r.message, r2.message), r2.partialResult))), lifecycle.add(second.lifecycle));
						})
						.orElseGet(
		...
		public <A, B, R> App<DataResult.Mu, R> ap2(final App<DataResult.Mu, BiFunction<A, B, R>> func, final App<DataResult.Mu, A> a, final App<DataResult.Mu, B> b) {
			...
			if (fr.result.left().isPresent()
				...
				return new DataResult<>(Either.left(fr.result.left().get().apply(
						ra.result.left().get(),
						rb.result.left().get())), fr.lifecycle.add(ra.lifecycle).add(rb.lifecycle));
			}
		...
		public <T1, T2, T3, R> App<DataResult.Mu, R> ap3(final App<DataResult.Mu, Function3<T1, T2, T3, R>> func, final App<DataResult.Mu, T1> t1, final App<DataResult.Mu, T2> t2, final App<DataResult.Mu, T3> t3) {
			...
			if (fr.result.left().isPresent()
				...
				return new DataResult<>(Either.left(fr.result.left().get().apply(
						dr1.result.left().get(),
						dr2.result.left().get(),
						dr3.result.left().get())), fr.lifecycle.add(dr1.lifecycle).add(dr2.lifecycle).add(dr3.lifecycle));
			}
